...
public class DetailsActivity extends ListActivity implements SearchView.OnQueryTextListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Intent intent = getIntent();
		 extra = intent.getStringExtra(SendAlerts.EXTRA_STRING);
	...
	class DBAsyncTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			try{
				...
				if(params[0].equals("Criminal"))
				{
					 httppost = new HttpPost("http://testandroid.net46.net/getCriminalDetails.php");
				}
				else if(params[0].equals("Police"))
				{
					 httppost = new HttpPost("http://testandroid.net46.net/getPoliceDetails.php");
					}
			...
			try{
				...
				for(int i=0; i<jArray.length();i++){
					JSONObject json = jArray.getJSONObject(i);
						if(extra.equals("Criminal"))
						{
							 detailsLIst[i] =  json.getString("NAME");

						}
						else if(extra.equals("Police"))
						{
							 detailsLIst[i] =  json.getString("POST") +" "+ json.getString("PLACE") ;
						}
					}
				setListAdapter(new ArrayAdapter<String>(DetailsActivity.this,android. R.layout.simple_list_item_1, detailsLIst));
